 ...
 */
const publishToDouyin = async (args: any) => {
    const { content, media_urls, credentials } = args;
    const client_key = credentials?.client_key || process.env.DOUYIN_CLIENT_KEY;
    const client_secret = credentials?.client_secret || process.env.DOUYIN_CLIENT_SECRET;
    const access_token = credentials?.access_token || process.env.DOUYIN_ACCESS_TOKEN;
    const open_id = credentials?.open_id || process.env.DOUYIN_OPEN_ID;

    if (!access_token || !open_id) {
        return { success: false, message: "Missing Douyin access_token or open_id (check env DOUYIN_ACCESS_TOKEN, DOUYIN_OPEN_ID)" };
    }
 ...
 */
const publishToWeChat = async (args: any) => {
    const { title, content, author, credentials, should_publish } = args;
    const app_id = credentials?.app_id || process.env.WECHAT_APP_ID;
    const app_secret = credentials?.app_secret || process.env.WECHAT_APP_SECRET;

    if (!app_id || !app_secret) {
        return { success: false, message: "Missing WeChat app_id or app_secret (check env WECHAT_APP_ID, WECHAT_APP_SECRET)" };
    }
 ...
 */
const publishToXiaohongshu = async (args: any) => {
    const { title, content, image_urls, credentials } = args;
    const access_token = credentials?.access_token || process.env.XIAOHONGSHU_ACCESS_TOKEN;

    if (!access_token) {
        return { success: false, message: "Missing Xiaohongshu access_token (check env XIAOHONGSHU_ACCESS_TOKEN)" };
    }
 ...
 */
const publishToBilibili = async (args: any) => {
    const { title, content, category, credentials } = args;
    const sessdata = credentials?.sessdata || process.env.BILIBILI_SESSDATA;
    const bili_jct = credentials?.bili_jct || process.env.BILIBILI_BILI_JCT;

    if (!sessdata || !bili_jct) {
        return { success: false, message: "Missing Bilibili SESSDATA or BILI_JCT (check env BILIBILI_SESSDATA, BILIBILI_BILI_JCT)" };
    }
 ...
 * Uses twitter-api-v2.
 */
const publishToTwitter = async (args: any) => {
    const { content, media_urls, credentials } = args;
    const app_key = credentials?.app_key || process.env.TWITTER_APP_KEY;
    const app_secret = credentials?.app_secret || process.env.TWITTER_APP_SECRET;
    const access_token = credentials?.access_token || process.env.TWITTER_ACCESS_TOKEN;
    const access_secret = credentials?.access_secret || process.env.TWITTER_ACCESS_SECRET;

    if (!app_key || !app_secret || !access_token || !access_secret) {
        return { success: false, message: "Missing Twitter credentials (check env TWITTER_APP_KEY, TWITTER_APP_SECRET, TWITTER_ACCESS_TOKEN, TWITTER_ACCESS_SECRET)" };
    }
 ...
 */
const publishToFeishu = async (args: any) => {
    const { title, content, credentials } = args;
    const app_id = credentials?.app_id || process.env.FEISHU_APP_ID;
    const app_secret = credentials?.app_secret || process.env.FEISHU_APP_SECRET;

    if (!app_id || !app_secret) {
        return { success: false, message: "Missing Feishu credentials (check env FEISHU_APP_ID, FEISHU_APP_SECRET)" };
    }
    ...
    try {
        ...
        if (!docId) throw new Error("Feishu Doc ID not returned");

        // 2. Add Content
        // Note: Adding content requires constructing complex Block objects.
        // For this version, we return the created document URL.
        // Users can implement full block parsing in future iterations.

        return {
            success: true,
            doc_id: docId,
            url: `https://feishu.cn/docs/${docId}`,
            message: `Created Feishu Doc: ${title}. (Content population skipped in basic implementation)`
        };
